...
final class ChatViewModel: ObservableObject {
    ...
    func switchToLocalTestSession(id: String) {
        let key = stateKey(for: id)
        if let state = sessionStates[key] {
            restoreState(state)
    ...
    func applyTestEvent(_ event: GatewayEvent, sessionID eventSessionID: String) {
        let key = keyForIncomingEvent(sessionID: eventSessionID)
        var state = sessionStates[key] ?? ChatSessionState(
        ...
        publishVisibleStateIfNeeded(for: key, previousVisibleKey: visibleStateKey())
    }

    func bindRuntimeSession(displayID: String, runtimeID: String) {
        stableSessionByGatewayID[runtimeID] = displayID
        if var state = sessionStates[runtimeID] {
            state.sessionID = runtimeID
            sessionStates[displayID] = state
            sessionStates.removeValue(forKey: runtimeID)
        } else if sessionStates[displayID] == nil {
            sessionStates[displayID] = ChatSessionState(
                sessionID: runtimeID,
                messages: [],
                isStreaming: false,
                streamingMessageID: nil,
        ...
        if sessionID == runtimeID || sessionID == displayID {
            sessionID = runtimeID
            snapshotCurrentSession()
        }
    }
    ...
    var testCachedMessageCount: Int {
        let key = visibleStateKey() ?? sessionID ?? ""
        return sessionStates[key]?.messages.count ?? 0
    }

    func simulateTestResumeResult(displayID: String, runtimeID: String, history: [ChatMessage]) {
        stableSessionByGatewayID[runtimeID] = displayID
        if var existing = sessionStates[displayID] {
            let hasLiveTurn = existing.isStreaming || existing.streamingMessageID != nil || !existing.activeToolCalls.isEmpty || existing.pendingApproval != nil
            if !hasLiveTurn {
                existing.sessionID = runtimeID
                existing.messages = history
            }
            sessionStates[displayID] = existing
        } else {
            sessionStates[displayID] = ChatSessionState(
                sessionID: runtimeID,
                messages: history,
