...
private let log = Logger(subsystem: "com.researchours.HermesNative", category: "SRSStore")

/// Persists flashcard decks with SRS state to local disk.
/// Files live in Application Support/hermes-native/srs-decks/<id>.json
...
final class SRSStore {
    ...
    private init() {
        guard let appSupport = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
            ...
            srsDir = URL(fileURLWithPath: "/tmp/hermes-native/srs-decks")
            try? fileManager.createDirectory(at: srsDir, withIntermediateDirectories: true)
            return
        }
        srsDir = appSupport.appendingPathComponent("hermes-native/srs-decks", isDirectory: true)
        try? fileManager.createDirectory(at: srsDir, withIntermediateDirectories: true)
    }
